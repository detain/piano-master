...
## 0. Record meta
- song_id:        `prelude-in-c-l2`
- title (canonical): Prelude in C major, BWV 846 (Well-Tempered Clavier Book I, No. 1)
...
## 1. Identify the work
- Composer / attribution:            Johann Sebastian Bach
- Catalogue no. (BWV / K. / WoO / Op. / P. / H.): BWV 846 (Well-Tempered Clavier Book I, No. 1)
- Composition year: 1722
- Source for composition year:       candidates-2026-08.md row 10 ("1722 / **1801** Hoffmeister"); IMSLP work page "Prelude and Fugue in C major, BWV 846"

## 2. Composer death year → EU life+70 check
- Composer death year:               1750 (source: candidates-2026-08.md row 10, P0.8 research 2026-08-22, verified against the IMSLP work page)
- EU PD date: 1 Jan (death_year + 71) =  1821  →  PD in EU: [x] yes (if ≤ current year) [ ] no → FLAG
...
- France war-extension check (French composers / works first published in France):
    - n/a — German composer; first publication by Hoffmeister, not in France.
...
- If multiple authors (composer + librettist): use the LAST survivor's death year (EU joint-work rule).
    - n/a — instrumental keyboard prelude, no librettist. (Gounod's "Ave Maria" overlay is a separate work — see §6.)

## 3. First publication → USA rule
- First publication year:            1801 — first edition, Hoffmeister (first publication of the Well-Tempered Clavier Book I)
- First publication source (IMSLP "First Publication" field / publisher archive / WorldCat): candidates-2026-08.md row 10; IMSLP work page "Prelude and Fugue in C major, BWV 846"
...
- Posthumous publication nuance: first publication year may differ from composition year — record BOTH.
    - Composition 1722, first publication 1801 — posthumous first publication (51 years after the composer's death, d. 1750); recorded. 1801 ≤ 1930, so no USA issue; well beyond the 25-year first-publication-right window.

## 4. Edition provenance (the edition trap, §8.5.3)
- Source type:  [x] IMSLP scan   [ ] Mutopia typeset   [ ] other (archive.org / library scan)
- Work page URL: https://imslp.org/wiki/Prelude_and_Fugue_in_C_major,_BWV_846_(Bach,_Johann_Sebastian) (WTC Book I work page also carries scans: https://imslp.org/wiki/Well-Tempered_Clavier,_Book_1,_BWV_846-869_(Bach,_Johann_Sebastian))
- **File/scan URL** (specific file page or PDF URL): TO CONFIRM at ingest — PD scans on the standalone work page (per candidates row 10)
...
- Edition:  publisher, year, plate no.: 1801, Hoffmeister (first edition of WTC Book I) — city/plate TO CONFIRM at ingest
...
- For each: rule applied and result (life+70 / pre-1931 / longer terms: ES 80, MX 100, CO 80…).
    - US: first publication 1801 ≤ 1930 → pre-1931 rule → PD
    - EU (DE, FR, ES…): life+70 → PD since 1 Jan 1821; Spain 80-year → PD since 1831
    - UK / CA / AU: life+70 → PD
    - JP: life+70 → PD
    - BR: life+70 → PD; IN: life+60 → PD (all clear by death-year arithmetic, d. 1750)
- Any jurisdiction still restricted?  [x] none   [ ] list + geo-gate decision:
- territory_flags_json (per §8.5.6 schema):
  ```json
  {
    "US":  {"rule": "first publication <= 1930 (1801 Hoffmeister)", "pd": true},
    "EU":  {"rule": "life+70 (d. 1750)", "pd": true, "note": "PD from 1 Jan 1821; ES 80-year clear 1831"},
...
- Any elements copied from a modern arrangement?  [x] none (required)
    - Do-not-blend: Gounod's "Ave Maria" overlay on BWV 846 is a separate work (PD: Gounod d. 1893) — do not blend (candidates row 10).
- Lyrics used? lyricist + death year (e.g., Schiller 1805 / Newton 1807):  none (instrumental prelude) → n/a
- Audio: rendered from our own arrangement (§8.6), no third-party recordings:  [x] confirmed

## 7. Verdict
- [x] GLOBALLY PD        (rule that establishes it: Bach d. 1750; first pub. 1801 → USA pre-1931)
