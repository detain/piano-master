...
## 0. Record meta
- song_id:        `scarborough-fair-l1-2`
- title (canonical): Scarborough Fair (Child Ballad No. 2, "The Elfin Knight")
...
## 1. Identify the work
- Composer / attribution:            Traditional English ballad, "The Elfin Knight" (Child Ballad No. 2); anonymous
- Catalogue no. (BWV / K. / WoO / Op. / P. / H.): Child Ballad No. 2 ("The Elfin Knight")
- Composition year: 17th c. ballad
- Source for composition year:       candidates-2026-08.md row 15 ("Traditional English (Child Ballad No. 2, 'The Elfin Knight')", "17th c. ballad"); IMSLP work page "Scarborough Fair (Folk Songs, English)" (reference only)

## 2. Composer death year → EU life+70 check
- Composer death year:               n/a — anonymous traditional English ballad ("The Elfin Knight", Child Ballad No. 2); no composer recorded (candidates row 15)
- EU PD date: n/a — traditional ballad, no life+70 arithmetic (candidates row 15 basis: "EU: traditional → PD")  →  PD in EU: [x] yes (if ≤ current year) [ ] no → FLAG
...
- France war-extension check (French composers / works first published in France):
    - n/a — English traditional ballad; collected English sources, not first published in France.
...
- If multiple authors (composer + librettist): use the LAST survivor's death year (EU joint-work rule).
    - n/a — traditional ballad; no known authors. (S&G's 1966 overlay is copyrighted — see §6.)

## 3. First publication → USA rule
- First publication year:            17th–19th c. broadsides/collections — collected publications: Child, *English and Scottish Popular Ballads* (1882–98) and Cecil Sharp, *Folk Songs from Somerset* (1904–09); exact earliest print TO CONFIRM at ingest
- First publication source (IMSLP "First Publication" field / publisher archive / WorldCat): candidates-2026-08.md row 15; IMSLP work page (reference only)
- USA status: published in **1930 or earlier** → PD in USA (even if URAA-eligible)   [x] yes [ ] no
    - Pre-1931 ✓ (traditional tune; candidates row 15).
...
- Posthumous publication nuance: first publication year may differ from composition year — record BOTH.
    - n/a — anonymous traditional ballad; no composition/publication split to record.

## 4. Edition provenance (the edition trap, §8.5.3)
- Source type:  [ ] IMSLP scan   [ ] Mutopia typeset   [x] other (archive.org / library scan) — PD collections on archive.org (candidates row 15)
- Work page URL: https://imslp.org/wiki/Scarborough_Fair_(Folk_Songs,_English) — reference only (2014 modern typeset, NOT a clean source)
- **File/scan URL** (specific file page or PDF URL): TO CONFIRM at ingest — Cecil Sharp, *Folk Songs from Somerset* (1904–09) or Child, *English and Scottish Popular Ballads* (1882–98) scan on archive.org (candidates row 15)
- IMSLP file ID (e.g., `#02607`) / Mutopia ID (e.g., `Mutopia-2008/01/13-1247`): n/a — archive.org scans; no IMSLP file ID (the IMSLP page is a 2014 modern typeset, arranger alive → NOT a clean source)
- Edition:  publisher, year, plate no.: Cecil Sharp, *Folk Songs from Somerset*, 1904–09; or Child, *English and Scottish Popular Ballads*, 1882–98 — publisher/city TO CONFIRM at ingest
- Editor (if any) + editor death year:     Collector/editor: Cecil Sharp (Folk Songs from Somerset) or Francis James Child (English and Scottish Popular Ballads) — death years TO CONFIRM at ingest (not recorded in candidates file); both collections PD per candidates row 15 (19th/early-20th-c.)
- IMSLP/Mutopia license tag on the file:  [x] Public Domain   [ ] CC-BY   [ ] CC0   [ ] CC-BY-SA → REJECT (§8.5 tier 3)   [ ] Non-PD US / Non-PD EU → REJECT
    - Archive.org scans of the 1882–98 / 1904–09 collections — PD (pre-1931 publications).
...
- Is this a modern *arrangement* by a living/recent arranger?  [ ] no [x] yes → REJECT as source (reference only)
    - The IMSLP page is a 2014 modern typeset (arranger alive) → NOT a clean source; we source the tune from the PD collections on archive.org instead (candidates row 15).
...
- For each: rule applied and result (life+70 / pre-1931 / longer terms: ES 80, MX 100, CO 80…).
    - US: pre-1931 ✓ (traditional tune) → PD
    - EU (DE, FR, ES…): traditional → PD (anonymous ballad)
...
- territory_flags_json (per §8.5.6 schema):
  ```json
  {
    "US":  {"rule": "traditional tune, pre-1931", "pd": true},
    "EU":  {"rule": "traditional (anonymous ballad)", "pd": true, "note": "no life+70 arithmetic"},
    "UK":  {"rule": "traditional (anonymous ballad)", "pd": true},
    "CA":  {"rule": "traditional (anonymous ballad)", "pd": true},
    "AU":  {"rule": "traditional (anonymous ballad)", "pd": true},
...
- Any elements copied from a modern arrangement?  [x] none (required)
    - Do-not-copy: S&G's 1966 arrangement (incl. the "Canticle" overlay) is copyrighted; the IMSLP page (2014 modern typeset) is reference only (candidates row 15).
- Lyrics used? lyricist + death year (e.g., Schiller 1805 / Newton 1807):  traditional ballad text (Child No. 2, "The Elfin Knight") — traditional → PD? [x] yes [ ] no (flag/omit)
    - The S&G 1966 "Canticle" counter-melody/text is copyrighted — never used (candidates row 15).
- Audio: rendered from our own arrangement (§8.6), no third-party recordings:  [x] confirmed

## 7. Verdict
- [x] GLOBALLY PD        (rule that establishes it: Traditional Child ballad — PD everywhere (with arrangement note))
