...
## 0. Record meta
- song_id:        `the-entertainer-l3`
- title (canonical): The Entertainer
...
## 1. Identify the work
- Composer / attribution:            Scott Joplin
- Catalogue no. (BWV / K. / WoO / Op. / P. / H.): n/a (ragtime piano piece; no catalogue system)
- Composition year: 1902
- Source for composition year:       candidates-2026-08.md row 12 ("1902 / **1902** Stark"); IMSLP work page "The Entertainer (Joplin, Scott)"

## 2. Composer death year → EU life+70 check
- Composer death year:               1917 (source: candidates-2026-08.md row 12, P0.8 research 2026-08-22, verified against the IMSLP work page)
- EU PD date: 1 Jan (death_year + 71) =  1988  →  PD in EU: [x] yes (if ≤ current year) [ ] no → FLAG
...
- France war-extension check (French composers / works first published in France):
    - n/a — American composer; first edition published by Stark (USA), not in France.
...
- If multiple authors (composer + librettist): use the LAST survivor's death year (EU joint-work rule).
    - n/a — instrumental ragtime piano piece, no librettist.

## 3. First publication → USA rule
- First publication year:            1902 — first edition, Stark
- First publication source (IMSLP "First Publication" field / publisher archive / WorldCat): IMSLP work page "First Publication" field; candidates-2026-08.md row 12
...
- Posthumous publication nuance: first publication year may differ from composition year — record BOTH.
    - Composition 1902, first publication 1902 — within the composer's lifetime (d. 1917); no posthumous nuance.

## 4. Edition provenance (the edition trap, §8.5.3)
- Source type:  [x] IMSLP scan   [ ] Mutopia typeset   [ ] other (archive.org / library scan)
- Work page URL: https://imslp.org/wiki/The_Entertainer_(Joplin,_Scott)
- **File/scan URL** (specific file page or PDF URL): TO CONFIRM at ingest — the 1902 Stark first-edition scan, **file #02607**, on the work page
- IMSLP file ID (e.g., `#02607`) / Mutopia ID (e.g., `Mutopia-2008/01/13-1247`): **#02607** (1902 Stark first-edition scan)
- Edition:  publisher, year, plate no.: 1902, Stark (first edition) — city/plate TO CONFIRM at ingest. A 1988 Schott urtext (ed. Voigt) is also hosted on the page (EU typographical right expired 2013) but the 1902 scan is cleanest (candidates row 12).
...
- Critical/urtext edition?  [x] no [ ] yes → confirm ≥25 years since publication (EU typographical right) or use an original scan instead
    - The 1988 Schott urtext (ed. Voigt), if ever referenced, satisfies the ≥25-year typographical rule (expired 2013); we use the 1902 Stark first-edition scan.
- Is this a modern *arrangement* by a living/recent arranger?  [x] no [ ] yes → REJECT as source (reference only)
    - Do-not-copy: Hamlisch's 1973 arrangement from *The Sting* is copyrighted (candidates row 12) — reference only, never copied.
...
- For each: rule applied and result (life+70 / pre-1931 / longer terms: ES 80, MX 100, CO 80…).
    - US: first publication 1902 ≤ 1930 → pre-1931 rule → PD
    - EU (DE, FR, ES…): life+70 → PD since 1 Jan 1988; Spain 80-year → PD since 1998
    - UK / CA / AU: life+70 → PD
    - JP: life+70 → PD
    - BR: life+70 → PD; IN: life+60 → PD (all clear by death-year arithmetic, d. 1917)
- Any jurisdiction still restricted?  [x] none   [ ] list + geo-gate decision:
- territory_flags_json (per §8.5.6 schema):
  ```json
  {
    "US":  {"rule": "first publication <= 1930 (1902 Stark)", "pd": true},
    "EU":  {"rule": "life+70 (d. 1917)", "pd": true, "note": "PD from 1 Jan 1988; ES 80-year clear 1998"},
...
- Any elements copied from a modern arrangement?  [x] none (required)
    - Do-not-copy: Hamlisch's 1973 *Sting* arrangement is copyrighted (candidates row 12) — reference only, never copied.
- Lyrics used? lyricist + death year (e.g., Schiller 1805 / Newton 1807):  none (instrumental rag) → n/a
- Audio: rendered from our own arrangement (§8.6), no third-party recordings:  [x] confirmed

## 7. Verdict
- [x] GLOBALLY PD        (rule that establishes it: Joplin d. 1917 → EU PD 1988; pub. 1902 Stark → USA pre-1931)
