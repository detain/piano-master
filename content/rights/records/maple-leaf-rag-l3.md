...
## 0. Record meta
- song_id:        `maple-leaf-rag-l3`
- title (canonical): Maple Leaf Rag
...
- Source for composition year:       candidates-2026-08.md row A1 ("1899 Stark"); IMSLP work page "Maple Leaf Rag (Joplin, Scott)"

## 2. Composer death year → EU life+70 check
- Composer death year:               1917 (source: candidates-2026-08.md row A1, P0.8 research 2026-08-22, verified against the IMSLP work page)
...
- First publication year:            1899 — first edition, Stark
- First publication source (IMSLP "First Publication" field / publisher archive / WorldCat): IMSLP work page "First Publication" field; candidates-2026-08.md row A1
...
- Posthumous publication nuance: first publication year may differ from composition year — record BOTH.
    - Composition 1899, first publication 1899 — within the composer's lifetime (d. 1917); no posthumous nuance.

## 4. Edition provenance (the edition trap, §8.5.3)
- Source type:  [x] IMSLP scan   [ ] Mutopia typeset   [ ] other (archive.org / library scan)
- Work page URL: https://imslp.org/wiki/Maple_Leaf_Rag_(Joplin,_Scott)
- **File/scan URL** (specific file page or PDF URL): TO CONFIRM at ingest — the 1899 Stark first-edition scan on the work page (candidates row A1 pins the work page but not a specific file ID)
- IMSLP file ID (e.g., `#02607`) / Mutopia ID (e.g., `Mutopia-2008/01/13-1247`): TO CONFIRM at ingest — no file ID pinned in candidates row A1; use the 1899 Stark first-edition scan tagged Public Domain on the work page
- Edition:  publisher, year, plate no.: 1899, Stark (first edition) — city/plate TO CONFIRM at ingest
...
- For each: rule applied and result (life+70 / pre-1931 / longer terms: ES 80, MX 100, CO 80…).
    - US: first publication 1899 ≤ 1930 → pre-1931 rule → PD
...
- territory_flags_json (per §8.5.6 schema):
  ```json
  {
    "US":  {"rule": "first publication <= 1930 (1899 Stark)", "pd": true},
...
- Any elements copied from a modern arrangement?  [x] none (required)
    - No modern arrangement flagged in candidates row A1 — verify none is used at ingest.
...
- [x] CLEARED-WITH-NOTE  (note: candidates-2026-08.md row A1 establishes GLOBALLY PD on the rule "Joplin d. 1917 → EU PD 1988; pub. 1899 Stark → USA pre-1931"; filed here as CLEARED-WITH-NOTE per the alternates record process — human second-person review required at the P0.8.4 gate before treating as cleared.)
