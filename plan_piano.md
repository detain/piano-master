...
**Next:** push + CI 5/5; OAF checkpoint→TFLite export attempt pending (checkpoint at /home/sites/maestro/oaf/, magenta 2.1.4 `--no-deps` in conda `py311-oaf`, TF restore in progress — result unknown, document both outcomes); P0.3.6 model ADR once numbers complete (ADR-0003 landed `2af30e8` — see continuation.md); P0.6.3 8h idle soak PASSED 20:06Z (runbook soak-results-2026-08-26.md; ledger Task 19); hardware-gated tasks unchanged.

### 2026-08-26 — P1.6 lesson player implemented (D1–D4, 4 commits)

**Commits:** `0b28af0` (D1), `5226130` (D2), `1ca619b` (D3), `1c52e85` (D4). On master; push pending CI.

**What shipped:**
- **D1 — RealtimeScorer** in `:scoring`: incremental driver over the batch `Scorer`; freeze rule watermark > `runningMaxClose[k]` (prefix freeze, never flips); tentative = full batch given events so far; `finalize()` == batch EXACTLY; delivery contract (monotone `onTimeNs`, tick after all events <= now); frozenScore NOT monotone (documented); tempo-inversion regression test (M beat 7.9 close 8.26, N beat 8.01 close 8.0925, wrong-pitch at 8.10 — prefix invariant); last-ulp boundary tests (0.5+0.18 rounds down vs 680000000/1e9 rounds up — freeze and Matcher complementary on the same doubles); property suite 200 seeds × 8 scenarios incl. frozen==batch-now gate. Verified: 134 tests, LINE 99.52%, jacoco >=0.95 gate green.
- **D2 — data/session layer:** SongPack v1 model (`com.keyquest.app.songpack`) + org.json loader (format gate `songpack/v1`, strict required fields, spec ranges incl. denominator set 1,2,4,8,16,32, strictly-increasing tempoMap) + `LayoutHintDeriver` (pitch-rank lanes % 5 per hand, per-staff <=0.5-beat beam runs, xHint = startBeat*1000) + `ProtoScoreAdapter` (chunk filter [startBeat,endBeat), rebased renderer beats + rebased xHint, tieToIndex pack→chunk-local remap with cross-chunk → null, tempo fallback, absolute-beat expected notes) + `LessonSession` (READY/PLAYING/PAUSED/FINISHED; frame-clock anchored pass clock; loop = reset-and-replay with re-anchor; retry rebuilds scorer; combo advances on event AND clock freezes in freeze order) + `ComboTracker`. Verified: 72 app tests.
- **D3 — lesson player UI:** `LessonPlayerScreen` (virtual frame clock — single source of truth, freezes while paused; pass-relative renderer beats + keyboard target window; per-frame NoteFeedback arrays zero-alloc; transport: play/pause/loop/tempo/progress/skin toggle/reduced motion/combo readout; results overlay stars/score/heatmap/retry/next; fail-loud asset load) + `OnScreenKeyboard` (multi-touch `awaitEachGesture`, boundary-centered black keys, ~1-beat target glow, wrong-key flash, KeyboardLayout pure math) + `NoteVoice` seam (`SilentVoice`) + renderer feedback wiring (verdict colors, hit pop 0.25-beat decay, staff color-only) + `externalSongTimeBeats` + `pickup_anacrusis` bundled as asset + `MainActivity` → `LessonPlayerScreen`. Verified: 80 app tests.
- **D4 — Paparazzi screenshots + CI:** Paparazzi 2.0.0-alpha05 — 4 goldens vs the `pickup_anacrusis` fixture (both skins w/ feedback, keyboard targets, results overlay — deterministic component-level; the full screen has async asset loading + frame loop). Build workarounds: buildscript sdk-common 31.13.2→31.9.1 force (31.13.2 REMOVED `GradleVersion.parse` breaking AGP 8.9.1 `VersionCheckPlugin`); app-module kotlin-stdlib 2.2.0 force (paparazzi transitives resolve 2.3.0; compiler is 2.2.0). Renderer testability seams: `layoutDispatcher` (Unconfined in tests), `fixedViewportSize` (Paparazzi never fires `onSizeChanged` — verified empirically), `produceState` keys include `fixedViewportSize`. CI android-unit += `:app:verifyPaparazziDebug`. Verified: 84 tests green locally (80 unit + 4 screenshots); goldens recorded on this Linux/JDK21 server.
- **Open items:** CI cross-machine golden verification on first push; device-side fps/latency measurement when hardware arrives; P1.8.4 tempo control (multi-point tempo renderer mapping), P1.8 mic clock alignment (`NoteEvent.onTimeNs` must be on the frame clock — TOUCH path only today), loop-replay detection via frozenCount regression (zero-note chunk latent).
